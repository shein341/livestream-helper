...
这个项目的目标不是单纯“能调通 LLM”，而是让面试官能看到一条可解释、可验证、可调优的 RAG 流水线。实现过程中主要做了以下取舍。
...
- 优点：首字符能在模型开始生成时立即到达，符合题目对 SSE 的要求。
- 缺点：服务端代码需要处理上游 SSE 解析、异常和日志落盘。
- 结论：面试场景下真实流式更有说服力，伪流式只适合临时演示，不适合作为最终实现。
